...
class PuzzleTile(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.image is not None and self.value != 0:
            ...
            if not self.is_goal:
                # Draw number at bottom-right corner, smaller and with outline
                font_size = max(12, self.size // 7)
                font = QFont(FONT_FAMILY, font_size, QFont.Weight.Bold)
                painter.setFont(font)
                padding = max(6, self.size // 18)
                number = str(self.value)
                # Calculate text size
                metrics = painter.fontMetrics()
                text_width = metrics.horizontalAdvance(number)
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        if dx != 0 or dy != 0:
                            ...
                            painter.drawText(x + dx, y + dy, number)
                # Draw main number (deep blue)
